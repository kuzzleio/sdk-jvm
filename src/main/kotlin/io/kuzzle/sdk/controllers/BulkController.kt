...
@JvmOverloads
  fun deleteByQuery(
    ...
    return kuzzle
        .query(query)
        .thenApplyAsync { response -> (response.result as ConcurrentHashMap<String?, Any?>)["deleted"] as LazilyParsedNumber).toInt()}
  }
...
@JvmOverloads
  fun import(
      index: String,
      collection: String,
      bulkData: ConcurrentHashMap<String, Any?>,
      queuable: Boolean? = null): CompletableFuture<ConcurrentHashMap<String, Any?> {
    val query = KuzzleMap().apply {
      put("index", index)
      put("collection", collection)
      put("controller", "bulk")
